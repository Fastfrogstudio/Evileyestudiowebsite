...
/**
 * A recipe's contributions to the shared deferred-reveal hook.
 *
 * Separate from applyOverridePatches because these APPEND into two generated
 * methods rather than adding methods of their own — that is what lets several
 * board-writing mechanics coexist without any of them owning the reveal.
 */
function applyBoardHooks(gameDir, hooks) {
	if (!hooks.length) return;
	ensureBoardHooks(gameDir);
	const file = path.join(gameDir, 'game_override.py');
	let source = fs.readFileSync(file, 'utf8');
	for (const hook of hooks) {
		const result = appendToMethod(source, hook.method, hook.body, hook.id);
		if (!result.replaced) {
			throw new Error(
				`game_override.py has no ${hook.method}() for "${hook.id}" to append to.`,
			);
		}
		source = result.source;
 ...
 */
/**
 * The one place a drawn board is rewritten before the player is shown it.
 *
 * Board mechanics used to be spliced in right AFTER `self.draw_board(...)` in
 * run_spin/run_freespin — but draw_board emits the reveal at its END, so every
 * one of them wrote to the board after the client had already been told what the
 * board was. Measured on a generated sticky-multiplier-wild game: 3,930 of 8,312
 * wins paying a multiplier above 1x — 47% — were not explained by the board the
 * client received. One round paid a 36x multiplier on a reveal showing no
 * multiplier wild at all.
 *
 * So the reveal is deferred instead: draw_board is overridden once, every
 * mechanic appends into apply_board_mechanics(), and the reveal goes out after
 * they have all run. Annotations that describe what a mechanic did go in
 * emit_board_mechanic_events(), which runs after the reveal — the client needs
 * the board before it can be told what landed on it.
 */
const BOARD_HOOK_METHODS = `    def draw_board(self, emit_event: bool = True, trigger_symbol: str = "scatter") -> None:
        """Draw the board, let every board mechanic rewrite it, THEN reveal it.

        GENERATED by stake-forge. The reveal is deferred deliberately: a mechanic
        that writes to the board after reveal_event() has fired pays out on cells
        the client was never shown.
        """
        super().draw_board(emit_event=False, trigger_symbol=trigger_symbol)
        self.apply_board_mechanics()
        if emit_event:
            reveal_event(self)
            self.emit_board_mechanic_events()

    def apply_board_mechanics(self) -> None:
        """Every mechanic that rewrites the drawn board, before the reveal."""

    def emit_board_mechanic_events(self) -> None:
        """Annotations describing what those mechanics did, after the reveal."""
`;

/**
 * Put the deferred-reveal hook into game_override.py once, whatever asked for it.
 * Idempotent: two mechanics both needing it must not generate it twice.
 */
function ensureBoardHooks(gameDir) {
	const file = path.join(gameDir, 'game_override.py');
	let source = fs.readFileSync(file, 'utf8');
	if (!/def apply_board_mechanics\(/.test(source)) {
		source = appendMethodsToClass(source, 'GameStateOverride', BOARD_HOOK_METHODS, 'apply_board_mechanics').source;
	}
	source = ensureImport(source, 'src.events.events', ['reveal_event']).source;
	fs.writeFileSync(file, source, 'utf8');
}
...
function applyBoardMechanics(gameDir, spec, mechanic) {
	...
	for (const { definition, config } of enabled) {
		...
		let sites = 0;
		if (definition.site === 'after-cascade') {
		...
		} else if (/^\s*self\.draw_board\(/m.test(gamestate)) {
			// after-draw: into the deferred-reveal hook, NOT after the draw_board
			// call — that call emits the reveal, so anything spliced after it
			// rewrites a board the client has already been sent.
			// Flush first: ensureBoardHooks reads the file, and the method this
			// mechanic just appended is still only in memory.
			fs.writeFileSync(overrideFile, override, 'utf8');
			ensureBoardHooks(gameDir);
			override = fs.readFileSync(overrideFile, 'utf8');
			const hooked = appendToMethod(
				override,
				'apply_board_mechanics',
				[`        ${definition.call}`],
				`board:${definition.id}:apply`,
			);
			if (hooked.replaced) {
				override = hooked.source;
				sites += 1;
			}
		}
		if (!sites) {
