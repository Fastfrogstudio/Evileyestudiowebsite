...
import { externalResources } from '../src/lib/externalResources.js';
import {
...
import { patchScreenComponents, SCREEN_PLACEHOLDER_KEYS } from '../src/lib/screenPatch.js';
import { renderBackdrop, renderBoardFrame } from '../src/lib/placeholderScreens.js';
import { unsupportedChars } from '../src/lib/font5x7.js';
...
test('an external stylesheet is caught, and TexturePacker metadata is not', () => {
	// Stake's approval notes: a build must be static files only, and external
	// resources beyond their CDN fail approval. The sample every game is cloned
	// from loads a font from use.typekit.net, so this ships inherited — and it
	// resolves fine in development, which is exactly why it survives to
	// submission.
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ext-'));
	fs.mkdirSync(path.join(dir, 'src'), { recursive: true });
	fs.writeFileSync(
		path.join(dir, 'src', 'app.html'),
		'<head>\n<link rel="stylesheet" href="https://use.typekit.net/aba0ebl.css" />\n</head>\n',
	);
	// Inert metadata inside a sprite atlas — parsed, never fetched. Flagging it
	// would bury the one finding that matters under a dozen that do not.
	fs.writeFileSync(
		path.join(dir, 'src', 'atlas.css'),
		'/* generated by https://www.codeandweb.com/texturepacker */\n.a{color:red}\n',
	);
	// A same-origin asset is not external.
	fs.writeFileSync(path.join(dir, 'src', 'ok.svelte'), '<img src="/assets/logo.png" />\n');

	const found = externalResources(dir);
	assert.equal(found.length, 1, `expected exactly the typekit link, got ${JSON.stringify(found)}`);
	assert.match(found[0].url, /use\.typekit\.net/);
	assert.equal(found[0].line, 2, 'the line number must point at the link');
	fs.rmSync(dir, { recursive: true, force: true });
});

test('build output is not scanned twice', () => {
	// build/ is generated FROM src/, so reporting both doubles every finding and
	// points at a file nobody edits.
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ext2-'));
	for (const sub of ['src', 'build', 'node_modules']) fs.mkdirSync(path.join(dir, sub), { recursive: true });
	const markup = '<link rel="stylesheet" href="https://example.com/x.css" />\n';
	fs.writeFileSync(path.join(dir, 'src', 'a.html'), markup);
	fs.writeFileSync(path.join(dir, 'build', 'a.html'), markup);
	fs.writeFileSync(path.join(dir, 'node_modules', 'a.html'), markup);
...
const SPINE_BACKGROUND = `<script lang="ts">
	import { SpineProvider, SpineTrack } from 'pixi-svelte';
</script>
<SpineProvider key="foregroundAnimation"><SpineTrack animationName={'idle'} /></SpineProvider>
`;
const SPINE_BOARD_FRAME = `<script lang="ts">
	import { SpineProvider } from 'pixi-svelte';
</script>
<SpineProvider key="reelhouse" />
`;

function screenApp() {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-screen-'));
	const components = path.join(dir, 'src', 'components');
	fs.mkdirSync(components, { recursive: true });
	fs.writeFileSync(path.join(components, 'Background.svelte'), SPINE_BACKGROUND);
	fs.writeFileSync(path.join(components, 'BoardFrame.svelte'), SPINE_BOARD_FRAME);
	return dir;
}

test('an undelivered background swaps the component to a sprite', () => {
	// Without the swap the game does not fall back to nothing — it falls back to
	// the sample app it was cloned from, and reads as that game.
	const dir = screenApp();
	const result = patchScreenComponents(dir, { screens: {} });
	const background = fs.readFileSync(path.join(dir, 'src', 'components', 'Background.svelte'), 'utf8');

	assert.deepEqual(result.swapped.sort(), ['Background.svelte', 'BoardFrame.svelte']);
	assert.ok(!background.includes('foregroundAnimation'), "the sample's spine key must be gone");
	assert.ok(background.includes(SCREEN_PLACEHOLDER_KEYS.basegame), 'it must draw our backdrop');
	assert.ok(background.includes(SCREEN_PLACEHOLDER_KEYS.freegame), 'both game types');
	fs.rmSync(dir, { recursive: true, force: true });
});

test('delivering the art restores the spine component verbatim', () => {
	// A one-way strip would mean the tool could never render real background art
	// again once it had swapped in a placeholder. This is the property that makes
	// the swap safe to do automatically.
	const dir = screenApp();
	patchScreenComponents(dir, { screens: {} });
	const swapped = fs.readFileSync(path.join(dir, 'src', 'components', 'Background.svelte'), 'utf8');
	assert.ok(swapped.includes('GENERATED by stake-forge'));

	const result = patchScreenComponents(dir, {
		screens: { 'background.basegame': { skeleton: 'a.json', atlas: 'a.atlas' }, 'background.freegame': { skeleton: 'b.json', atlas: 'b.atlas' } },
	});
	assert.deepEqual(result.restored, ['Background.svelte']);
	assert.equal(
		fs.readFileSync(path.join(dir, 'src', 'components', 'Background.svelte'), 'utf8'),
		SPINE_BACKGROUND,
...
test('a half-delivered background stays on the placeholder', () => {
	// One component serves both game types. Taking the spine version with only
	// the base background delivered would render the sample's art for the base
	// game and ours for the feature.
	const dir = screenApp();
	patchScreenComponents(dir, { screens: {} });
	patchScreenComponents(dir, { screens: { 'background.basegame': { skeleton: 'a.json', atlas: 'a.atlas' } } });
	const background = fs.readFileSync(path.join(dir, 'src', 'components', 'Background.svelte'), 'utf8');
	assert.ok(background.includes('GENERATED by stake-forge'), 'must not half-restore');
	fs.rmSync(dir, { recursive: true, force: true });
});

test('re-running the swap does not re-stash or churn the file', () => {
	// A second stash would overwrite the pristine original with a generated
	// placeholder and make the swap permanent — the exact failure the stash
	// exists to prevent.
	const dir = screenApp();
	patchScreenComponents(dir, { screens: {} });
	const stash = path.join(dir, 'src', 'components', '.forge-original', 'Background.svelte');
	assert.equal(fs.readFileSync(stash, 'utf8'), SPINE_BACKGROUND);

	const second = patchScreenComponents(dir, { screens: {} });
	assert.deepEqual(second.swapped, [], 'a re-run is a no-op');
	assert.equal(fs.readFileSync(stash, 'utf8'), SPINE_BACKGROUND, 'the stash must still be the ORIGINAL');
	fs.rmSync(dir, { recursive: true, force: true });
});

test('the board-frame placeholder keeps the glow subscription alive', () => {
	// bookEventHandlerMap broadcasts boardFrameGlowShow/Hide. Deleting the
	// listener makes that broadcast throw at runtime instead of degrading.
	const dir = screenApp();
	patchScreenComponents(dir, { screens: {} });
	const frame = fs.readFileSync(path.join(dir, 'src', 'components', 'BoardFrame.svelte'), 'utf8');
	assert.ok(frame.includes('boardFrameGlowShow'), 'the subscription must survive');
	assert.ok(frame.includes('boardFrameGlowHide'));
	assert.ok(frame.includes('EmitterEventBoardFrame'), 'the exported type is imported elsewhere');
	// The generated file NAMES reelhouse in a comment explaining why the glow is
	// gone, so the check is that no spine is rendered, not that the word is absent.
	assert.ok(!frame.includes('SpineProvider'), "the sample's glow spine must not be rendered");
	assert.ok(!frame.includes('SpineTrack'));
	fs.rmSync(dir, { recursive: true, force: true });
});

test('backdrops differ between base and feature, and are real PNGs', () => {
	// "Did the background change when free spins started" is a real thing to
	// check; identical placeholders would hide it.
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-backdrop-'));
	const base = path.join(dir, 'base.png');
	const feature = path.join(dir, 'feature.png');
	fs.writeFileSync(base, renderBackdrop({ variant: 'basegame' }));
	fs.writeFileSync(feature, renderBackdrop({ variant: 'freegame' }));

	assert.deepEqual(readPngSize(base), { width: 1200, height: 1200 }, 'must decode as a PNG');
	assert.notDeepEqual(
		fs.readFileSync(base),
		fs.readFileSync(feature),
		'the two game types must be visually distinguishable',
	);

	// The frame is rendered at the BOARD's aspect, not square, or the corner
	// ticks distort when it is stretched to a 5x3 board.
	const framePath = path.join(dir, 'frame.png');
	fs.writeFileSync(framePath, renderBoardFrame({ reels: 5, rows: 3, cell: 100 }));
	const size = readPngSize(framePath);
	assert.ok(size.width > size.height, `a 5x3 frame must be landscape, got ${size.width}x${size.height}`);
	assert.equal(size.width / size.height > 1.4, true, 'aspect must track the board');
	fs.rmSync(dir, { recursive: true, force: true });
});

test('a frame label with an undrawable character is refused, not silently mangled', () => {
	// drawText substitutes '?' for anything the 5x7 font lacks, and it lacks ':'.
	// The first version of this label shipped as "PLACEHOLDER? BOARDFRAME" into a
	// real screenshot with nothing anywhere explaining why.
	assert.throws(
		() => renderBoardFrame({ reels: 5, rows: 3, cell: 100, label: 'placeholder: boardFrame' }),
		/cannot draw/,
		'a colon must be rejected',
	);
	assert.deepEqual(unsupportedChars('placeholder: boardFrame'), [':']);
	assert.deepEqual(unsupportedChars('PLACEHOLDER - BOARDFRAME'), []);
	// The label the tool actually uses must be drawable.
	assert.doesNotThrow(() => renderBoardFrame({ reels: 5, rows: 3, cell: 100, label: 'placeholder - boardframe' }));
});
